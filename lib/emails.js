...
// Postal address for the footer. Transactional mail does not need one, but
// marketing mail does under CAN-SPAM, and the nurture sequence renders its
// footer from here too. Keeping one definition means the address cannot be
// correct in one template and stale in another.
...
/** Shared footer block. `reason` says why this person is receiving the email. */
function renderFooter(reason) {
  return `<div style="font-size:12px;color:#888;line-height:1.5;">
      ${POSTAL_ADDRESS[0]} &middot; <a href="https://midly.ai" style="color:#888;">midly.ai</a><br>
      ${POSTAL_ADDRESS[1]}<br>
      ${POSTAL_ADDRESS[2]}<br><br>
      ${reason}
    </div>`;
}
...
<body style="margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;background:#f4f4f2;padding:32px 16px;line-height:1.6;color:#0a0a0a;">
  <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e8e8e4;border-radius:12px;padding:36px 32px;">
    ...
    ${renderFooter("You're receiving this because you submitted a form on midly.ai.")}
  </div>
</body></html>`;
...
module.exports = { renderNotification, renderAutoresponse, renderFooter, POSTAL_ADDRESS };
